...
const UI = {
  orgName: '社團法人測試市藥師公會', orgShort: '測試公會', systemName: 'UCC 排班系統',
...
const PERMKEYS = [['lock','班表鎖定'],['notes','編輯注意事項'],['skip','標記逾期放棄'],['create','建立月份分頁'],
  ['openday','編輯開放日'],['resetpw','使用者密碼重設'],['roster','增減人員'],['manage','代管班表']].map(x=>({k:x[0],label:x[1]}));
...
run('pharm', [], (w, D) => {
  ...
  const ft = T(D, 'ftOrg') + '|' + T(D, 'ftSys') + '|' + T(D, 'ftConf') + '|' + T(D, 'ftContact');
  ok(ft === [UI.orgName, UI.systemName, UI.confidentialNote, UI.contactLine].join('|'), '頁尾四段', ft);
  ...
  run('super', [], (w2, D2, calls2) => {
    ...
    ok(subs.length === 8 && subs.every(vis), '8 個分項全部顯示', subs.map(s => s.getAttribute('data-perm') + (vis(s) ? '' : '✗')));
    // 展開權限授權 → 抓名單
    ...
    setTimeout(() => {
      ...
      ok(boxes.length === 8 && boxes.filter(b => b.checked).map(b => b.value).join() === 'lock', '會務人員目前只勾 lock', boxes.filter(b => b.checked).map(b => b.value));
